...
|---------|-------|------|-------------|
| POST | `/nestvia/auth/login` | Non | Connexion (email + password) → JWT + refresh token |
| POST | `/nestvia/auth/refresh` | Non | Regénérer un JWT à partir d'un refresh token |
...
| `REFRESH_TOKEN_DAYS` | Durée de validité du refresh token (jours) | `30` |
...
2. Le serveur vérifie les identifiants, puis renvoie un **JWT** (valide `JWT_EXPIRES_IN`, défaut `24h`) et un **refresh token** (valide `REFRESH_TOKEN_DAYS`, défaut `30` jours).
3. Pour chaque requête protégée, le client inclut le JWT dans le header `Authorization: Bearer <token>`.
...
5. Quand le JWT expire, le client appelle `POST /nestvia/auth/refresh` avec le `refresh_token` pour obtenir un nouveau JWT et un nouveau refresh token **(rotation automatique)**.
...
**2. Regénérer un token expiré :**

```bash
curl -X POST https://api.leofranz.fr/nestvia/auth/refresh \
  -H "Content-Type: application/json" \
